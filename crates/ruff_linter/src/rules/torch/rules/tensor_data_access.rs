...
pub(crate) fn tensor_data_access(checker: &Checker, attribute: &ast::ExprAttribute) {
    ...
    // Avoid flagging when `.data` is itself the callee of a call (e.g., `x.data()`).
    // Note: we must *not* skip when `.data` merely appears as an argument
    // (e.g., `foo(x.data)`), which also has a `Call` as its parent expression.
    if let Some(Expr::Call(call)) = semantic.current_expression_parent()
        && call.func.range() == attribute.range()
    {
        return;
    }
