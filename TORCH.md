...
| TORCH008 | Explicit `device` missing in `torch.tensor()` call                    | No      |
| TORCH009 | Device mismatch: mixing CPU and CUDA tensors in an op                 | No      |
| TORCH010 | `torch.tensor(t)` copies data; use `t.clone()` to avoid extra alloc   | No      |
| TORCH011 | `.clone()` on gradient-carrying tensor without `.detach()`            | No      |
| TORCH012 | Use `.to(device/dtype)` instead of `.cpu()`, `.cuda()`, `.float()`    | Yes     |
| TORCH013 | `.squeeze()` without `dim` may silently drop unexpected dimensions    | No      |
| TORCH014 | Avoid `.tensor().to(device)`; create tensor directly on target device | Yes     |
...
| TORCH206 | `weight_norm` deprecated; use `torch.nn.utils.parametrizations.weight_norm`      | Yes     |
| TORCH207 | `torch.arange` with float step may produce inconsistent element count            | No      |
| TORCH208 | Use `torch.linalg.*` instead of deprecated `torch.*` linear algebra functions    | Yes     |
| TORCH209 | Use `register_buffer`/`register_parameter` for non-parameter module state        | No      |
| TORCH210 | Use `ParameterList`/`ParameterDict` instead of plain `list`/`dict` for params    | No      |
| TORCH211 | Don't apply `softmax` before `CrossEntropyLoss`; it includes log-softmax itself  | No      |
| TORCH212 | `BatchNorm` with `batch_size=1` produces `NaN`; use `GroupNorm` or `LayerNorm`   | No      |
| TORCH213 | Don't call `.forward()` directly; invoke the module as a callable                | No      |
...
| TORCH303 | Missing `worker_init_fn` to seed RNG in multi-worker `DataLoader`  | No      |
...
| TORCH504 | `torch.cat`/`torch.stack` in a loop; accumulate in a list and call once | No      |
...
| TORCH604 | `torch.load` without `map_location`; may silently move tensors to wrong device | No      |
| TORCH605 | `torch.hub.load` with `trust_repo=False` or missing trust argument             | No      |
| TORCH606 | `torch.hub.load` without pinning a commit hash is not reproducible             | No      |
